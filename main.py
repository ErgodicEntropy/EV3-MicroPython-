...
class Node:
    ...
    def direction_to_neighbor(self, node):
        if self.is_neighbor(node):
            return [k for k, n in self.__neighbors.items() if n == node][0] #returns key (direction: str)
        return None
...
if __name__ == '__main__':
    ...
    path = [g.node_of('A'), g.node_of('B'), g.node_of('C'), g.node_of('D')] #list of Grid-Node classes type 
    for idx, node in enumerate (path[1:]):
        source = node
        sink = node.neighbor('right')
        DIR = source.direction_to_neighbor(sink)
        if DIR == 'up':
            d = robot.distance()
            while d < 290:
...
#---------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    path = [g.node_of('A'), g.node_of('B'), g.node_of('C'), g.node_of('D')] #list of Grid-Node classes type 
# Creer la boucle for 
    DM = np.zero((4,4)) #Distance Matrix
    DIM = np.zero((4,4)) #Direction Matrix
    m = 0
    for idx, node in enumerate (path[m:]): # [(0,path[m]),(1,path[m+1]),(2,path[m+2])]
        t = m
        while t < len(path):
            dist = node.distance_to_neighbor(path[t+1])
            direction = node.direction_to_neighbor(path[t+1])
            DM[m][t+1] = dist
            DM[t+1][m] = dist
            DIM[m][t+1] = direction
            t +=1
...
#Greedy algorithm implementation (source-to-sink mode) 
   def Greedy(a:Node,b:Node):
       distance = 0
       node_chain = [a]
       GL = []
       GD = []
       for k in range(len(DM[a])):
           if DM[a][k] != 0:
               GD.append(DM[a][k])
        MIN_INDEX = np.argmin(GD)[0] #index of the minimum distance
        distance += DM[a][MIN_INDEX]
        ndd = DIM[a][MIN_INDEX] #new node direction
        nn = a.neighbor(ndd) # new node
        node_chain.append(nn)
        if nn == b:
            break
        else:
            distance += distance + Greedy(nn,b)
            return node_chain, distance
        
    
    #Greedy Steering Control
    dirdict = {'up':0, 'right':180, 'left':-180, 'down':360} #view-from-above reference frame
    
    Node1 = g.node_of('A')
    Node2 = g.node_of('B')
    
    NC = Greedy(Node1,Node2)[0]
    ##Assumption: Black Lines are diagonal and rectangular (not just rectangular) --> check below for the other version where this doesn't holds
    k = 1
    IN = NC[k] #intermediary node (if it exists)
    DIR = Node1.direction_to_neighbor(IN)
    robot.turn(dirdict[DIR])
    d = robot.distance()
    while d < 290:
        deviation = line_sensor.reflection() - threshold
        turn_rate = PROPORTIONAL_GAIN * deviation
...
#Dynamic Programming implementation (source-to-sink): Bellman equation <=> F(n) = min(L(n)+F(n+1))
    def Bellman(a:Node,b:Node):
        pass
